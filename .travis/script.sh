...
then
	docker run -t -v "${PWD}:/libfsrefs" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libfsrefs && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libfsrefs;
