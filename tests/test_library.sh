...
LIBRARY_TESTS="block_descriptor block_descriptors directory error io_handle level0_metadata level1_metadata level2_metadata level3_metadata metadata_block metadata_table metadata_value notify volume_name";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libfsrefs" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
