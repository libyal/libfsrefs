/*
 * Library level0_metadata type test program
 *
...
#include "../libfsrefs/libfsrefs_level0_metadata.h"

#if defined( __GNUC__ ) && !defined( LIBFSREFS_DLL_IMPORT )

/* Tests the libfsrefs_level0_metadata_initialize function
 * Returns 1 if successful or 0 if not
 */
int fsrefs_test_level0_metadata_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libfsrefs_level0_metadata_initialize(
	          &level0_metadata,
	          &error );
	...
	level0_metadata = (libfsrefs_level0_metadata_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsrefs_level0_metadata_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsrefs_level0_metadata_initialize with memset failing
		 */
...
/* Tests the libfsrefs_level0_metadata_free function
 * Returns 1 if successful or 0 if not
 */
int fsrefs_test_level0_metadata_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	FSREFS_TEST_RUN(
	 "libfsrefs_level0_metadata_initialize",
	 fsrefs_test_level0_metadata_initialize );

	FSREFS_TEST_RUN(
	 "libfsrefs_level0_metadata_free",
	 fsrefs_test_level0_metadata_free );

	/* TODO: add tests for libfsrefs_level0_metadata_read */
